...
class Deque {
    ...
    Deque() {
        ...
        frontIndex = -1;
        rearIndex = -1;
        for (int i{}; i < capacity; i++) arr[i] = 0;
    }
    ...
    Deque(std::initializer_list<T> l) {
        ...
        arr = new T[capacity];
        int i{};
        for (auto it{l.begin()}; it != l.end(); it++) arr[i++] = *it;
        frontIndex = 0;
        rearIndex = size - 1;
    }
    ...
    Deque(Deque<T> const &dq) {
        ...
        arr = new T[capacity];
        for (int i{}; i < capacity; i++) arr[i] = dq.arr[i];
    }
    ...
    void assign(int const count, T data) {
        ...
        for (int i{}; i < count; i++) arr[i] = data;
        frontIndex = 0;
        rearIndex = size - 1;
    }
    ...
    void push_front(T data) {
        ...
        if (size == capacity) {
            ...
            for (int i{}; i<newCapacity; i++) narr[i] = 0;
            // 1. If frontIndex < rearIndex
            if (frontIndex < rearIndex) {
                for (int i{frontIndex}, j{}; i <= rearIndex; i++, j++) narr[j] = arr[i];
                frontIndex = 0;
            ...
            // 2. If frontIndex > rearIndex
                ...
                int i{frontIndex}, j{};
                for(; i < capacity; i++) narr[j++] = arr[i];
                i = 0;
                for (; i <= rearIndex; i++) narr[j++] = arr[i];
                frontIndex = 0;
                rearIndex = capacity - 1;
            }
