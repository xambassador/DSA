...
void printLevelWise(TreeNode<int>* root) {
    ...
    while(!pendingTreeNodes.empty()) {
        ...
        for(int i{}; i < front->childrens.size(); i++) {
            ...
            if(i == front->childrens.size() - 1) cout << front->childrens[i]->data;
            else cout << front->childrens[i]->data << ",";
        }
        cout << endl;
        pendingTreeNodes.pop();
    }
...
void print(TreeNode<int>* root) {
    cout << root->data << " :";
    for(int i{}; i < root->childrens.size(); i++) cout << root->childrens[i]->data << ",";
    cout << endl;

    // Subtrees are printed by recursion
    for(int i{}; i < root->childrens.size(); i++) print(root->childrens[i]);
}
