...
int squareBrackets(int openingBrackets, int closingBrackets, int totalBrackets, bool* arr) {
    if (openingBrackets > totalBrackets || closingBrackets > totalBrackets) {
        return 0;
    } 
    ...
    int ans {};
    if (openingBrackets == closingBrackets || arr[openingBrackets + closingBrackets + 1]) {
        ans = squareBrackets(openingBrackets+1, closingBrackets,totalBrackets,arr);
    } else if (openingBrackets == totalBrackets) {
        ans = squareBrackets(openingBrackets,closingBrackets+1, totalBrackets,arr);
    } else {
...
int squareBrackets(int openingBrackets, int closingBrackets, int totalBrackets, bool* arr, int dp[21][21]) {
    ...
    int ans {};
    if (openingBrackets == closingBrackets || arr[openingBrackets + closingBrackets + 1]) {
        ans = squareBrackets(openingBrackets+1, closingBrackets,totalBrackets,arr,dp);
    } else if (openingBrackets == totalBrackets) {
        ans = squareBrackets(openingBrackets,closingBrackets+1, totalBrackets,arr,dp);
    } else {
