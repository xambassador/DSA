...
int bruteForce(int** grid, int srow, int scol, int row, int col) {
    ...
    // I return 1
    if((srow == row - 1 && scol == col) || (srow == row && scol == col - 1)) return 1;

    // Now if i cross grid
    if(srow > row || scol > col) return INT_MAX;
...
int help_dp(int** grid, int srow, int scol, int row, int col) {
    ...
    for(int i{col - 3}; i >= 0; --i) {
        int tmpAns{1};
        if(grid[row - 1][i + 1] < 0) tmpAns = dp[row - 1][i + 1] + abs(grid[row - 1][i + 1]);
        else tmpAns = dp[row - 1][i + 1] - grid[row - 1][i + 1];
    ...
    for(int i{row - 3}; i >= 0; --i) {
        int tmpAns{1};
        if(grid[i][col - 1] < 0) tmpAns = dp[i + 1][col - 1] + abs(grid[i + 1][col - 1]);
        else tmpAns = dp[i + 1][col - 1] - grid[i + 1][col - 1];
        if(tmpAns <= 0) tmpAns = 1;
        dp[i][col - 1] = tmpAns;
    }
    ...
    for(int r{row - 2}; r >= 0; --r) {
        for(int c{col - 2}; c >= 0; --c){
            ...
            if(tmpAns1 <= 0) tmpAns1 = 1;
            if(tmpAns2 <= 0) tmpAns2 = 1;
