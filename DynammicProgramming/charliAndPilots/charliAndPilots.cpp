...
int minSalary(int* captain, int* assistant, int n, int X) {
    ...
    if (X == 0) ans = minSalary(captain+1, assistant+1, n-1, X+1) + assistant[0];
    else if (X == n) ans = minSalary(captain+1, assistant+1, n-1,X-1) + captain[0];
    else {
...
int minSalary(int* captain, int* assistant, int n, int X, int** dp) {
    ...
    int ans{};
    // x is 0, then current pilot is consider as younger, so he can't become captain, but he can become assistant
    if (X == 0) ans = minSalary(captain + 1, assistant + 1, n - 1, X + 1, dp) + assistant[0];
    else if (X == n) ans = minSalary(captain + 1, assistant + 1, n - 1, X - 1, dp) + captain[0];
    else {
