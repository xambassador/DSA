...
int help(BinarySearchTreeNode<int>* root, int sum) {
    ...
    int leftans {help(root->left, root->data)}, ans;
    if(sum == 0) ans = help(root->right,sum);
    else ans = help(root->right, sum);
    return ans;
}
/******************************************************************************************/
...
int Position(int key, vector<int> v, int start, int end) {
    ...
    if(key < v[mid]) return Position(key, v, start, mid - 1);
    if(key > v[mid]) return Position(key, v, mid + 1, end);
}
...
void help1(BinarySearchTreeNode<int>* root, vector<int>& v) {
    ...
    if(position < v.size() - 1) {
        for(int i{position + 1}; i < v.size(); i++) sum += v[i];
    }
    root->data = sum;
    help1(root->left,v);
    help1(root->right,v);
